...
public class ExpenseRepository implements ExpenseRepositoryInterface {
  ...
  " VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
  private static final String SQL_COUNT_BY_EMAIL = "SELECT COUNT(*) FROM exp_tracker_expense WHERE EMAIL = ?";
  ...
  "FROM exp_tracker_expense WHERE email = ?";
  ...
  @Override
  public List<Expense> findAll(String email) throws ExpenseResourceNotFoundException {
    return jdbcTemplate.query(SQL_FIND_BY_EMAIL, new Object[] { email }, expenseRowMapper);
  }
